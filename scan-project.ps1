...
function Generate-Report
{
    ...
    $high_risk = 0
    if ($null -ne $file_hash)
    {
        $high_risk += $file_hash.Count
    }
    ...
    if ($null -ne $versions)
    {
        $high_risk += $versions.Count
    }
    if ($null -ne $malicious_activity)
    {
        $high_risk += $malicious_activity.Count
    }

    $medium_risk = 0
    if ($null -ne $namespaces)
    {
        $medium_risk += $namespaces.Count
    }
    if ($null -ne $suspicious_patterns)
    {
        $medium_risk += $suspicious_patterns.Count
    }
    if ($null -ne $secret_scanning_patterns)
    {
        $medium_risk += $secret_scanning_patterns.Count
    }
    if ($null -ne $hooks)
    {
        $medium_risk += $hooks.Count
    }
...
function Main
{
    ...
    $InitializationScript = {
        ...
        function Run-DependencyAnalysis
        {
            ...
            {
                Write-Warn "package.json not found. Skipping dependency analysis."; return
            }
            Write-Info "Scanning package.json..."
            try
            {
                $pkgJson = Get-Content $pkg_file -Raw | ConvertFrom-Json
                $allDependencies = @{ }; if ($pkgJson.PSObject.Properties['dependencies'])
                {
                ...
                {
                    $found = $allDependencies.Keys | Where-Object { $_.StartsWith("$ns/") }; if ($found)
                    {
        ...
        function Scan-ForMaliciousFiles
        {
            ...
            {
                $file_hash = (Get-FileHash -Path $file.FullName -Algorithm SHA256).Hash.ToLower(); if ($file_hash -contains $MALICIOUS_HASH)
                {
        ...
        function Scan-ForHooks
        {
            ...
            foreach ($file in $jsonFiles)
            {
                try
                {
                    $content = Get-Content $file.FullName -Raw | ConvertFrom-Json -ErrorAction SilentlyContinue; if ($null -ne $content.scripts.postinstall)
                    {
        ...
        function Scan-Workflows
        {
            ...
            {
                Write-Info ".github/workflows directory not found, skipping."; return
            }
        ...
        function Scan-ForCorrelatedExfiltration
        {
            ...
            {
                $content = Get-Content $file.FullName -Raw -ErrorAction SilentlyContinue; if (($content -match $env_regex) -and ($content -match $exfil_regex))
                {
                    $file.FullName.Substring($project_path.Length + 1) | Out-File -FilePath $findings_file -Append -Encoding utf8
                }
            }
        ...
        function Scan-ForPatterns
        {
            ...
            {
                Select-String -Path $filesToScan.FullName -Pattern $regex -List -ErrorAction SilentlyContinue | ForEach-Object { "$findingPrefix $($_.Path.Substring($project_path.Length + 1) )" | Out-File -FilePath $findings_file -Append -Encoding utf8 }
            }
        ...
        function Analyze-GitState
        {
            ...
            {
                Write-Warn "Not a git repository, skipping."; return
            }
            try
            {
                "All local and remote branches:" | Out-File -FilePath $findings_file -Append -Encoding utf8; $branches = git -C $project_path branch -a; $branches | ForEach-Object { "  $_" } | Out-File -FilePath $findings_file -Append -Encoding utf8
            }
            catch
            {
